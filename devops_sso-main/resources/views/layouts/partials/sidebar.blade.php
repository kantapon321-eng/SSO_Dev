                    ...
                    @foreach($laravelAdminMenus->menus as $section)
                        @if(count(collect($section->items)) > 0)
                            @foreach($section->items as $menu)
                                @if($menu->title == 'counsels' && request()->getHost() == 'sso.tisi.go.th')
                                    @continue
                                @endif
                                @if($user->branch_type==2){{-- ถ้าเป็นสาขา --}}
                                    @if(!property_exists($menu, 'branch_block') || (property_exists($menu, 'branch_block') && $menu->branch_block!=1)){{-- เช็คว่าไม่ได้บล็อคสาขา --}}
                                        <li>
